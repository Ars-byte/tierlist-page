...
let activeTierMode = 'overall'; // overall, np, sw, cry, mc, dp, uhc, smp
...
// ==========================================================
// ICONOS LOCALES (ACTUALIZADO CON DP, UHC, SMP)
// ==========================================================
const ICONS = {
    sw: "assets/icons/sword.png", 
    np: "assets/icons/nethop.png", 
    cry: "assets/icons/vanilla.png", 
    mc: "assets/icons/mace.png",
    dp: "assets/icons/diamond_pot.png",
    uhc: "assets/icons/uhc.png",
...
function renderTierBadge(type, value) {
    ...
    // Si no existe icono para ese tipo (caso overall viejo o error), fallback
    if(!iconUrl) return ''; 
...
function renderOverall(dataList = players) {
    ...
    // Si hay búsqueda, SOLO se muestra la lista para no romper el podio visualmente con 1 solo elemento.
...
function renderTierColumns(mode, dataList = players) {
    ...
    tiersToShow.forEach(tierNum => {
        // Filtramos jugadores que tengan la propiedad del modo (ej: p.dp) y que esa propiedad incluya el numero de Tier
